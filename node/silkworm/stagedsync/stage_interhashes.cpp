...
Stage::Result InterHashes::regenerate_intermediate_hashes(db::RWTxn& txn, const evmc::bytes32* expected_root) {
    ...
    try {
        ...
        if (expected_root != nullptr && computed_root != *expected_root && node_settings_->chain_config.value().chain_id != kTelosEVMMainnetConfig.chain_id && node_settings_->chain_config.value().chain_id != kTelosEVMTestnetConfig.chain_id) {
            log_lck.lock();
