...
Stage::Result Senders::add_to_batch(BlockNum block_num, std::vector<Transaction>&& transactions) {
    ...
    for (const auto& transaction : transactions) {
        ...
        if (transaction.chain_id.has_value() && transaction.v() != intx::uint256{42}) {
        ...
        Bytes rlp{};
        rlp::encode(rlp, transaction, /*for_signing=*/false, /*wrap_eip2718_into_string=*/false);
